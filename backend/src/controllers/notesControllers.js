...
export async function getAllNotes(_,res) {
    try {
        const notes = await Note.find().sort({createdAt:-1});// shows the newest node first
...
export async function updateNote(req,res){
    try {
        ...
        if(!updateNote) return res.status.json({message:"note not found"})
...
export async function deleteNote(req,res){
    try {
        ...
        if(!deleteNote) return res.status.json({message:"note not found"});
